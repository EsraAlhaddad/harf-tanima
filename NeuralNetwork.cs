...
namespace YSA
{
    class NeuralNetwork
    {
        ...
        public NeuralNetwork(int girisKatmaniSayisi = 35, int cikisKatmaniSayisi = 5, int gizlikatmanSayisi = 5)
        {
            ...
            gizliKatmani = new GizliKatmani(gizlikatmanSayisi, cikisKatmaniSayisi);
            cikisKatmani = new CikisKatmani(cikisKatmaniSayisi);

        }

        // Öğrenmeyi yaptığımız yer 
        public bool Training(int[][] veriler,int[][] beklenenler) 
        {
            int epoch = 0;//kaç kez tüm veri setinin üzerinden geçileceğini belirler
        ...
        public double hataHesaplama(int[] beklenenler)
        {
            ...
            }//karesel hata (MSE) formülüdür.

            return hata;
        }

        public void ileriHesaplama(int[] veriler)//çıkış değerinin hesaplanması işlemi
        {
            // Giriş katmanının değeri ile ağırlığının çarpılarak toplanması ve toplanan sonuçların bias eklenerek sigmoid fonksiyonuna alıyor.
            // Daha sonra ise Çıkış value'ye atanıyor.
            //Gizli Katman İleri Hesaplama
            for (int i = 0; i < gizlikatmanSayisi; i++)
            {
                Noron noron = gizliKatmani.gizliKatmanNoronlari[i];
                double toplam = 0;
            ...
            // Gizli katmanının değeri ile ağırlığının çarpılarak toplanması ve toplanan sonuçların bias eklenerek sigmoid fonksiyonuna alıyor.
            // Daha sonra ise Çıkış value'ye atanıyor.
            //Çıkış Katmanı İleri Hesaplama
            for (int i = 0; i < cikisKatmani.cikisKatmanNoronlari.Length; i++)
            {
                ...
                {
                    Noron gizliNoron = gizliKatmani.gizliKatmanNoronlari[j];
        ...
        public void geriHesaplama(int[] veriler)
        {
            ...
            for(int i = 0; i < 5;i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    ...
                    gizliKatmani.gizliKatmanNoronlari[j].agirliklar[j] -= hata;
                    // Bias değerini hata değeri kadar azaltıyoruz
                    gizliKatmani.gizliKatmanNoronlari[j].bias -= hata;
                }
            }
            ...
            for (int i = 0; i < 35; i++)
            {
                girisKatmani.girisKatmanNoronlari[i].dagitilacakHata = gizliKatmani.gizliKatmanNoronlari[0].cikisValueOncesi *
            ...
            for (int i = 0; i < 35; i++)
            {
                hata = ogrenmeKatsayisi *
                       ...
                       gizliKatmani.gizliKatmanNoronlari[i % 5].agirliklar[0] +
